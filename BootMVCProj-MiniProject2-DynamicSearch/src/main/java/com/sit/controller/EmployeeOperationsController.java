...
public class EmployeeOperationsController {
	...
	public String deleteEmployeeDetails(@RequestParam("no")int no,
	   ...
	   @PostMapping("/search")
	    public String searchEmployees(@ModelAttribute("emp") Employee e, Model model) {
	        // Log the form data to ensure it's correctly bound
	        System.out.println("Form data received: " + e);

	        // Call service to search for employees with dynamic criteria
	        List<Employee> empList = empService.showEmployeeDynamicSearch(e);
	        ...
	        // Optional: Add a message to show search results
	        if (empList.isEmpty()) {
	            model.addAttribute("resultMsg", "No records found.");
	        } else {
	            model.addAttribute("resultMsg", "Search results");
	        }

	        // Return the view to show the results
	        return "show_report";  // or whatever your JSP page name is
	    }
	    ...
	    public List<Employee> showEmployeeDynamicSearch(Employee e) {
	        ...
	        if (e.getDeptno() == null || e.getDeptno() == 0) {
	            e.setDeptno(null);  // make deptno optional in search
	        }
	        
	        return empService.showEmployeeDynamicSearch(e);
	    }
